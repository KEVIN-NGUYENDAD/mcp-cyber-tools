...
$action1 = New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c `"$RepoPath\HOME_SOC_GOD_MODE.cmd`"" -WorkingDirectory $RepoPath
...
$action2 = New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c `"$RepoPath\collector.cmd`"" -WorkingDirectory $RepoPath
...
$action3 = New-ScheduledTaskAction -Execute "cmd.exe" -Argument "/c `"$RepoPath\report.cmd`"" -WorkingDirectory $RepoPath
...
Write-Host ""
Write-Host "Monitor collection: Get-Content $logPath\collector.log -Tail 10 -Wait"
Write-Host ""
